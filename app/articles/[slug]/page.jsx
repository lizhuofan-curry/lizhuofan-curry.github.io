...
export default async function ArticlePage({ params }) {
  ...
  return <main id="main-content" className="article-page page-shell"><Link className="back-link" href="/articles">← 返回文章</Link><div className="article-layout"><article className="reading-card"><header className="article-header"><p>{article.category} / {article.readingTime}{article.publishedAt && <> / <time dateTime={article.publishedAt}>发布于 {formatPublishedAt(article.publishedAt)}</time></>}</p><h1>{article.title}</h1><strong>{article.description}</strong><div className="large-tags">{article.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></header><div className="prose">{LocalContent ? <LocalContent /> : <MarkdownArticle>{article.body}</MarkdownArticle>}</div><footer className="article-end"><p>理解还会继续更新。</p><Link href="/articles">阅读其他文章</Link></footer></article><aside className="toc-card"><strong>文章目录</strong>{toc.length ? <nav>{toc.map((item) => <a href={`#${item.id}`} key={item.id}>{item.text}</a>)}</nav> : <p>这篇文章较短，可以直接阅读。</p>}<Link href="/articles">返回文章索引</Link></aside></div><RelatedLinks current={article} kind="article" /></main>;
}
