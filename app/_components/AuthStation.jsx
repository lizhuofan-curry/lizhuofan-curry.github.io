...
function PasswordField({ id, label, value, onChange, autoComplete = "current-password", autoFocus = false }) {
  const [visible, setVisible] = useState(false);
  return <label className="identity-field" htmlFor={id}><span>{label}</span><div className="password-control"><input id={id} type={visible ? "text" : "password"} value={value} onChange={onChange} minLength={8} maxLength={128} autoComplete={autoComplete} autoFocus={autoFocus} required /><button type="button" onClick={() => setVisible((current) => !current)} aria-label={visible ? "隐藏密码" : "显示密码"}>{visible ? <EyeSlash size={19} /> : <Eye size={19} />}</button></div></label>;
}

function AuthMessage({ status }) {
  if (!status) return null;
  return <p className={`identity-message ${status.kind}`} role={status.kind === "error" ? "alert" : "status"}>{status.text}</p>;
}
...
export function AuthStation({ mode = "login", next, emailEnabled = false, token }) {
  ...
  return <main id="main-content" className="identity-page page-shell"><section className="identity-station surface-card"><aside className="identity-signal" aria-hidden="true"><span>AUTH.EXE</span><strong>30</strong><i /><p>登录后保存<br />你的学习记录</p></aside><div className="identity-main"><p className="identity-kicker">身份站 / ZHUO</p>{mode === "forgot" ? <><h1>找回进入档案的钥匙。</h1><p className="identity-copy">输入邮箱后，如果账号存在，我们会发送一个可用的重置链接。</p><form className="identity-form" onSubmit={requestReset}><label className="identity-field"><span>邮箱</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" autoFocus required /></label><button className="primary-button" type="submit" disabled={busy}>{pending === "reset-request" && <SpinnerGap className="spin" size={18} />}发送重置链接</button></form><AuthMessage status={status} /><Link className="identity-text-link" href={loginHref}>返回登录</Link></> : mode === "reset" ? <><h1>设置一把新的钥匙。</h1><p className="identity-copy">新密码至少 8 位；完成后会撤销其他已登录设备。</p><form className="identity-form" onSubmit={resetPassword}><PasswordField id="new-password" label="新密码" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" autoFocus /><PasswordField id="confirm-password" label="确认新密码" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} autoComplete="new-password" /><button className="primary-button" type="submit" disabled={busy || !token}>{pending === "reset" && <SpinnerGap className="spin" size={18} />}更新密码</button></form><AuthMessage status={status} /><div className="identity-links"><Link className="identity-text-link" href="/forgot-password">重新申请重置链接</Link><Link className="identity-text-link" href={loginHref}>返回登录</Link></div></> : mode === "verify" ? <><h1>查收你的验证邮件。</h1><p className="identity-copy">验证后会自动登录，并安全返回你刚才所在的页面。</p><form className="identity-form" onSubmit={(event) => { event.preventDefault(); resendVerification(); }}><label className="identity-field"><span>邮箱</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" autoFocus required /></label><button className="secondary-button" type="submit" disabled={busy || emailUnavailable}>{pending === "verify" && <SpinnerGap className="spin" size={18} />}重新发送验证邮件</button></form><AuthMessage status={status} /><Link className="identity-text-link" href={loginHref}>返回登录</Link></> : <><nav className="identity-tabs" aria-label="账户入口"><Link aria-current={active === "login" ? "page" : undefined} href={loginHref}>登录</Link><Link aria-current={active === "register" ? "page" : undefined} href={registerHref}>注册</Link></nav><h1>{active === "login" ? "回到你的学习档案。" : "创建一份自己的档案。"}</h1><p className="identity-copy">{active === "login" ? "使用邮箱密码或 GitHub 登录。会话会安全保存，方便下次继续阅读和点赞。" : "注册后请验证邮箱，再开始记录你的学习与作品。"}</p><AuthMessage status={status} /><GitHubButton callbackURL={callbackURL} pending={pending} setPending={setPending} setStatus={setStatus} /><div className="identity-divider"><span>或使用邮箱</span></div>{emailUnavailable ? <div className="identity-disabled"><Key size={19} /><span>邮箱登录正在准备中，请先使用 GitHub 登录。</span></div> : active === "login" ? <form className="identity-form" onSubmit={handleEmailLogin}><label className="identity-field"><span>邮箱</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" autoFocus required /></label><PasswordField id="login-password" label="密码" value={password} onChange={(event) => setPassword(event.target.value)} /><button className="primary-button" type="submit" disabled={busy}>{pending === "login" && <SpinnerGap className="spin" size={18} />}使用邮箱登录</button><div className="identity-links"><Link className="identity-text-link" href="/forgot-password">忘记密码？</Link><Link className="identity-text-link" href={registerHref}>还没有账号？去注册</Link></div></form> : <form className="identity-form" onSubmit={handleRegister}><label className="identity-field"><span>显示名称</span><input value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" minLength={1} maxLength={80} autoFocus required /></label><label className="identity-field"><span>邮箱</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" autoFocus required /></label><PasswordField id="register-password" label="密码（至少 8 位）" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" /><PasswordField id="register-password-confirm" label="确认密码" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} autoComplete="new-password" /><small className="identity-hint">{password.length >= 8 ? "✓ 密码长度已满足" : "密码至少 8 位"}{confirmPassword ? (confirmPassword === password ? " · ✓ 两次一致" : " · 两次不一致") : ""}</small><label className="identity-consent"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} />我已阅读并同意 <Link href="/privacy" target="_blank">隐私说明</Link></label><button className="primary-button" type="submit" disabled={busy}>{pending === "register" && <SpinnerGap className="spin" size={18} />}创建并验证邮箱</button><div className="identity-links"><Link className="identity-text-link" href={loginHref}>已有账号？去登录</Link><Link className="identity-text-link" href={`/verify-email?next=${encodeURIComponent(callbackURL)}`}>重新发送验证邮件</Link></div></form>}<p className="identity-footnote">我们不会获得你的 GitHub 密码；邮箱仅用于认证与安全通知。</p></>}</div></section></main>;
}
