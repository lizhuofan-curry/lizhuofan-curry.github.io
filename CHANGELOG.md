...
- **feat/design**：首页完全重构为 daidr.me 风格四 Tab 可交互卡片——顶部 32px 圆角胶囊导航（我/文章/小项目/更多），「我」面板为 128px 圆形头像 + 名字 + 签名 + 圆形社交按钮（hover 扩散边框），「文章/小项目」面板为简洁列表/网格，「更多」面板为简介 + 标签 + 链接。涉及 `app/_components/HomeCard.jsx`、`app/globals.css`。
